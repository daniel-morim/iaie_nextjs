...
export async function POST(req: Request) {
    try {
        ...
        const { salesman_id, user_id } = await req.json();
        // Body obrigatório
        const body = {
            company_id: Number(company_id),
            salesman_id: Number(salesman_id),
        };
        //User id vai ser utilizado para alterar role do utilizador na nossa base de dados do website


        
        // Chamada ao endpoint
        const response = await fetch(`${apiUrl}/salesmen/delete/?access_token=${access_token}&json=true`, {
            method: "POST",
